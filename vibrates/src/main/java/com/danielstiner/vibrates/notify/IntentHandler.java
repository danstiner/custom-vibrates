...
public class IntentHandler implements IIntentHandler {
	...
	private static final String NOTIFY_PHONE = NotificationTypes
			.particularizeType(NotificationTypes.VOICE, "phone");
	...
	private void handlePhoneState(Bundle bundle, Context context) {
		...
		if (bundle.getString(TelephonyManager.EXTRA_STATE).equals(
			...
			String type = NotificationTypes.particularizeType(
					NotificationTypes.VOICE, NOTIFY_PHONE);

			notify_provider.get().identifier(international_number).type(type)
					.fire(context);
		}
	}
	...
	// Uri.withAppendedPath(CONVERSATIONS_URI, account), null,
	// "label:" + tagLabel, null, null);
	// cursor.moveToFirst();
	// String[] columns = cursor.getColumnNames(); // labelIds
	// String fromAddress = cursor.getString(cursor
	// .getColumnIndexOrThrow("fromAddress"));
	// String subject = cursor.getString(cursor
	// .getColumnIndexOrThrow("subject"));
	// cursor.close();
	//
	...
	// try {
	// foreignContext = context.createPackageContext(
	// "com.google.android.gm", Context.CONTEXT_INCLUDE_CODE
	...
	// String draftPluralString = "drafts"; //
	// context.getString(R.string.draftplural);
	// String sendingString = "sending"; // context.getString(R.string.sending);
	// String sendFailedString = "sendfailed"; //
	// context.getString(R.string.sendingfailed);
	...
	// try {
	// ms[i].invoke(null, instructions, from,
	// new SpannableStringBuilder(), maxChars,
	// unreadStyle, draftsStyle, meString,
	// draftString, draftPluralString, sendingString,
	...
	// // Earlier versions
	// try {
	// ms[i].invoke(null, instructions, from, maxChars,
	// unreadStyle, draftsStyle, meString,
	// draftString, draftPluralString, sendingString,
	// sendFailedString, forceAllUnread, forceAllRead);
	// } catch (IllegalArgumentException e) {
	...
	// notify_provider.get().identifier(fromAddress).type(type).extra(subject)
	// .fire(context);
	// // return new
	// // VibrateNotify(fromAddress).setType(type).setExtra(subject);
	// }
