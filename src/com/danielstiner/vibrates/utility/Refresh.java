...
public class Refresh {
	...
	public static void update(Entity entity,
		...
		else if(kind.equals(Entity.TYPE_GROUP))
		{
			...
			updateGroupMembers(entity, groupid, context.getContentResolver(), identifierManager);
		}
	}
	...
	private static String getGroupId(Entity entity, IIdentifierManager identifierManager) {
		// See if we have a single contacts contract id to use for this entity
		Cursor contactIds = null;
		try {
			contactIds = identifierManager.get(entity, IdentifierManager.KIND_CONTACTS_GROUP_ID);
	...
	private static void updatePhoneNumbers(Entity map_to_entity, String contactid, ContentResolver cr, IIdentifierManager identifierManager) {
		
		// Remove all old phone number identifiers
		identifierManager.removeAll(map_to_entity, NotificationTypes.VOICE_PHONECALL);
	...
	private static void addPhoneNumbers(Entity map_to_entity, String contactid, ContentResolver cr, IIdentifierManager identifierManager) {
		...
		while(!c.isAfterLast()) {
			...
			identifierManager.add(map_to_entity, number, NotificationTypes.VOICE_PHONECALL);
			identifierManager.add(map_to_entity, number, NotificationTypes.CHAT_TEXT);
	...
	private static void addDisplayNames(Entity map_to_entity, String contactid, ContentResolver cr, IIdentifierManager identifierManager) {
	...
	private static void updateGroupMembers(Entity group, String groupid,
			...
			try {
				info.moveToFirst();
			    // Grab some info
			    title = info.getString(info.getColumnIndexOrThrow(ContactsContract.Groups.TITLE));;
			    id = info.getLong(info.getColumnIndexOrThrow(ContactsContract.Groups._ID));
		    ...
		    identifierManager.add(group, title, IdentifierManager.KIND_CONTACTS_GROUP_TITLE);
		    identifierManager.add(group, id.toString(), IdentifierManager.KIND_CONTACTS_GROUP_ID);
			
		    
		    // remove 
		    Cursor members = contentResolver.query(
					ContactsContract.Data.CONTENT_URI,
					new String[] {
						ContactsContract.CommonDataKinds.GroupMembership.CONTACT_ID,
						ContactsContract.CommonDataKinds.GroupMembership.DISPLAY_NAME
						},
					ContactsContract.CommonDataKinds.GroupMembership.GROUP_ROW_ID + " = ? OR " +
			...
			if(members.getCount() < 1) return;
			
			// Add identifiers back
			members.moveToFirst();
			while(!members.isAfterLast()) {
				Long contactid = members.getLong(members.getColumnIndexOrThrow(ContactsContract.CommonDataKinds.GroupMembership.CONTACT_ID));
				
				// Add the identifiers for this contact
				addPhoneNumbers(group, contactid.toString(), contentResolver, identifierManager);
				
				// From our current query
				identifierManager.add(
						group,
						members.getString(members.getColumnIndexOrThrow(ContactsContract.CommonDataKinds.GroupMembership.DISPLAY_NAME)),
